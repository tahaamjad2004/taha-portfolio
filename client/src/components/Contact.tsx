...
export default function Contact() {
  // Use phoneSubject instead of subject to match backend schema
  const [formData, setFormData] = useState({ name: '', email: '', phoneSubject: '', message: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ name: '', email: '', phoneSubject: '', message: '' });
    ...
    <section id="contact" className="py-20 px-6 max-w-2xl mx-auto border-t border-slate-800 text-center">
      ...
      <form onSubmit={handleSubmit} className="text-left space-y-4">
        ...
        <div>
          ...
          <input type="text" required value={formData.phoneSubject} onChange={(e) => setFormData({...formData, phoneSubject: e.target.value})} className="w-full bg-slate-800 border border-slate-700 rounded px-4 py-2.5 text-slate-200 focus:outline-none focus:border-cyan-400 text-sm" />
        </div>
        <div>
